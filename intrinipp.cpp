...
AsmStatus asmAddC_32fc(const Asm32fc* pSrc, Asm32fc val, Asm32fc* pDst, int len)
{
	...
	// При проблемах с памятью попробовать заменить на storeu
	for (i; i < decr; i += 4)
	{
...
AsmStatus asmConvert_32f16u_Sfs(const Asm32f* pSrc, Asm16u* pDst, int len, AsmRoundMode rndMode, int scaleFactor)
{
	...
	{
		for (i = 0; i < decr / 2; i += 4)
		{
			_mm_stream_pi((__m64*) & pDst[i], _mm_cvtps_pi16(
				_mm_max_ps(_mm_set_ps(pSrc[i + 3], pSrc[i + 2], pSrc[i + 1], pSrc[i]), _mm_setzero_ps())));
			_mm_stream_pi((__m64*) & pDst[decr - i - 4], _mm_cvtps_pi16(
				_mm_max_ps(_mm_set_ps(pSrc[decr - i - 3], pSrc[decr - i - 2], pSrc[decr - i - 1], pSrc[decr - i]), _mm_setzero_ps())));
		}

		for (i; i < len; ++i)
		{
			pDst[i] = Asm16u(pSrc[i]);
		}
	}
	else if (rndMode == asmRndZero)
	{
		for (i = 0; i < decr / 2; i += 4)
		{
			...
			_mm_stream_pi((__m64*) & pDst[decr - i - 4], _mm_cvtps_pi16(_mm_round_ps(
				_mm_max_ps(_mm_set_ps(pSrc[decr - i - 3], pSrc[decr - i - 2], pSrc[decr - i - 1], pSrc[decr - i]), _mm_setzero_ps()), _MM_FROUND_TO_ZERO)));
		}

		for (i; i < len; ++i)
		{
			pDst[i] = Asm16u(pSrc[i] / 1);
		}
	}
	...
	if (rndMode == asmRndNear)
	{
		for (i = 0; i < decr / 2; i += 4)
		{
			_mm_stream_pi((__m64*) & pDst[i],
				_mm_cvtps_pi16(_mm_set_ps(pSrc[i + 3], pSrc[i + 2], pSrc[i + 1], pSrc[i])));
			_mm_stream_pi((__m64*) & pDst[decr - i - 4],
				_mm_cvtps_pi16(_mm_set_ps(pSrc[decr - i - 3], pSrc[decr - i - 2], pSrc[decr - i - 1], pSrc[decr - i])));
		}

		for (i; i < len; ++i)
		{
			pDst[i] = Asm16s(pSrc[i]);
		}
	}
	else if (rndMode == asmRndZero)
	{
		for (i = 0; i < decr / 2; i += 4)
		{
			...
			_mm_stream_pi((__m64*) & pDst[decr - i - 4], _mm_cvtps_pi16(_mm_round_ps(_mm_set_ps(pSrc[decr - i - 3], pSrc[decr - i - 2], pSrc[decr - i - 1], pSrc[decr - i]), _MM_FROUND_TO_ZERO)));
		}

		for (i; i < len; ++i)
		{
			pDst[i] = Asm16s(pSrc[i] / 1);
		}
	}
	//    else if(rndMode == asmRndFinancial)
	//        for (int i = 0; i < len/2; i += 4)
...
AsmStatus asmMean_32f(Asm32f* pSrc, int len, Asm32f* pMean, AsmHintAlgorithm hint)
{
	...
	// выглядит ужасно, но стало побыстрее
	...
	{
		mean = _mm_add_pd(
			_mm_add_pd( /*сложение суммы 8 элементов и среднего значения*/
				_mm_add_pd(_mm_cvtps_pd(_mm_loadu_ps(&pSrc[i])), _mm_cvtps_pd(_mm_loadu_ps(&pSrc[i + 2]))),  /*сложение первых 4 элементов*/
				_mm_add_pd(_mm_cvtps_pd(_mm_loadu_ps(&pSrc[i + 4])), _mm_cvtps_pd(_mm_loadu_ps(&pSrc[i + 6])))),  /*сложение следующих 4 элементов*/
			mean);
	}
...
AsmStatus asmFFTFwd_CToC_32fc_I(Asm32fc* pSrcDst, AsmFFTSpec_C_32fc* pFFTSpec, Asm8u* pBuffer)
{
	...
	if ((*pFFTSpec).buf_size == (*pFFTSpec).size && ((*pFFTSpec).flag == ASM_FFT_DIV_INV_BY_N || (*pFFTSpec).flag == ASM_FFT_DIV_BY_SQRTN))
	{
		...
		{
			_mm_storeu_ps(&pSrcDst[i].re, _mm_div_ps(_mm_setr_ps(pSrcDst[i].re, pSrcDst[i].im, pSrcDst[i + 1].re, pSrcDst[i + 1].im),
				_mm_set_ps1(div)));
			_mm_storeu_ps(&pSrcDst[i + 2].re, _mm_div_ps(_mm_setr_ps(pSrcDst[i + 2].re, pSrcDst[i + 2].im, pSrcDst[i + 3].re, pSrcDst[i + 3].im),
				_mm_set_ps1(div)));
		}
	}
